...
# ===================== КЛАВИАТУРЫ =====================
def create_main_keyboard(is_premium=False):
...
def create_profile_keyboard(is_premium=False):
    buttons = [
...
def create_viewing_keyboard(profile_id, is_premium=False):
    buttons = [
...
def create_like_keyboard(like_id, from_id):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⛔ Заблокировать", callback_data=f"block_from_like_{from_id}")]
    ])
    ...
    if user.get("photo"):
        try:
            return await message.answer_photo(
                user["photo"],
                caption=text,
                reply_markup=create_profile_keyboard(is_premium)
            )
        except Exception:
            await db.update_user(user_id, photo="", photo_updated=0)

    await message.answer(text, reply_markup=create_profile_keyboard(is_premium))
...
async def show_premium(message: Message, user_id: int):
    ...
    if is_premium:
        days_left = max(0, (int(user.get("premium_until", 0)) - int(time.time())) // 86400)
        text = (
        ...
        try:
            if candidate.get("photo"):
                await message.answer_photo(
                    candidate["photo"],
                    caption=text,
                    reply_markup=create_viewing_keyboard(candidate["id"], is_premium)
                )
            else:
                await message.answer(text, reply_markup=create_viewing_keyboard(candidate["id"], is_premium))
            return
        except TelegramBadRequest:
            # Битый file_id — очищаем фото и ищем следующую анкету
    ...
    for like in likes[:10]:
        ...
        if like.get("photo"):
            try:
                ...
                pass
        await message.answer(text, reply_markup=create_like_keyboard(like["like_id"], like["id"]))

    await db.mark_likes_seen(user_id)
...
# ===================== РЕГИСТРАЦИЯ =====================
async def start_flow(message: Message, state: FSMContext, user_id: int, first_name: str, username: Optional[str]):
    user = await db.get_user(user_id)
    ...
    if user:
        ...
        await message.answer(
            ...
            reply_markup=create_main_keyboard(is_premium)
        )
        return
...
async def process_gender(message: Message, state: FSMContext):
    ...
    await state.update_data(gender=message.text.split()[1])
    kb = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="👨 Мужской"), KeyboardButton(text="👩 Женский"), KeyboardButton(text="👥 Любой")]],
        resize_keyboard=True
    )
    await message.answer("Кого ты ищешь?", reply_markup=kb)
...
async def process_search_gender(message: Message, state: FSMContext):
    if message.text not in ["👨 Мужской", "👩 Женский", "👥 Любой"]:
        return await message.answer("❌ Выбери из кнопок")

    await state.update_data(search_gender=message.text.split()[1])
    kb = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="📍 Отправить геопозицию", request_location=True)]],
        resize_keyboard=True
    )
    await message.answer("Отправь свою геолокацию для поиска рядом:", reply_markup=kb)
...
async def process_bio(message: Message, state: FSMContext):
    ...
    has_profanity, _ = contains_profanity(message.text)
    if has_profanity:
        return await message.answer("❌ Описание содержит недопустимые слова")
    ...
    await message.answer("✅ Анкета создана! Добро пожаловать.", reply_markup=create_main_keyboard(is_premium))
...
async def start_edit(callback: CallbackQuery, state: FSMContext):
    ...
    if field == "name":
        ...
        await callback.message.answer("Отправь новую геолокацию:", reply_markup=kb)
...
async def save_edit_name(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Имя обновлено!", reply_markup=create_main_keyboard(await db.check_premium(message.from_user.id)))
...
async def save_edit_age(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Возраст обновлён!", reply_markup=create_main_keyboard(await db.check_premium(message.from_user.id)))
...
async def save_edit_bio(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Описание обновлено!", reply_markup=create_main_keyboard(await db.check_premium(message.from_user.id)))
...
async def save_edit_photo(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Фото обновлено!", reply_markup=create_main_keyboard(await db.check_premium(message.from_user.id)))
...
async def save_edit_location(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"✅ Город обновлён на: {html_escape(city)}", reply_markup=create_main_keyboard(await db.check_premium(message.from_user.id)))
...
async def save_edit_search_gender(message: Message, state: FSMContext):
    ...
    await db.update_user(
        message.from_user.id,
        search_gender=message.text.split()[1],
        last_active=int(time.time())
    )
    await state.clear()
    await message.answer("✅ Настройки поиска обновлены!", reply_markup=create_main_keyboard(await db.check_premium(message.from_user.id)))
...
async def handle_accept_like(callback: CallbackQuery):
    _, _, like_id, from_id = callback.data.split("_")
    from_id = int(from_id)
    user_id = callback.from_user.id
...
@dp.callback_query(F.data.startswith("block_from_like_"))
async def handle_block_from_like(callback: CallbackQuery):
    block_id = int(callback.data.removeprefix("block_from_like_"))
...
async def handle_block(callback: CallbackQuery):
    block_id = int(callback.data.removeprefix("block_"))
    await db.block_user(callback.from_user.id, block_id)
    await callback.answer("⛔ Заблокировано")
    with suppress(Exception):
        await callback.message.delete()
...
async def save_report_reason(message: Message, state: FSMContext):
    ...
    if ok:
        await message.answer("✅ Жалоба отправлена. Спасибо за помощь!", reply_markup=create_main_keyboard(await db.check_premium(message.from_user.id)))
        if is_admin(ADMIN_ID):
            await safe_execute(
                bot.send_message,
                ADMIN_ID,
        ...
        await message.answer("❌ Не удалось отправить жалобу.", reply_markup=create_main_keyboard(await db.check_premium(message.from_user.id)))
...
async def buy_premium(callback: CallbackQuery):
    await callback.answer()
    # Telegram Stars: provider_token должен быть пустым
...
async def successful_payment(message: Message):
    ...
    if payment.invoice_payload == "premium_pack":
        ...
        await message.answer(f"✅ Начислено {SUPERLIKE_PACK_COUNT} суперлайков!")
...
async def handle_admin(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "stats":
        ...
        if not users:
            await callback.message.edit_text("Нет зарегистрированных пользователей.", reply_markup=callback.message.reply_markup)
        else:
            lines = []
            for u in users:
...
async def admin_broadcast(message: Message, state: FSMContext):
    ...
    for user in users:
        if user.get("is_banned"):
            continue
        result = await safe_execute(bot.send_message, user["id"], f"📢 <b>Сообщение от администратора:</b>\n\n{html_escape(text)}")
        if result:
            sent += 1
        else:
            failed += 1
...
async def daily_reset_loop():
    # Сначала делаем сброс на старте, если нужно
